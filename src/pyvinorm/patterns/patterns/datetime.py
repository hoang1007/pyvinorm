...
class Time2Pattern(BaseTimePattern):
    def get_regex_pattern(self):
        ...
        return r"(?i)\b(?:2[0-4]|10|[01]?[1-9])\s?[:h]\s?[0-5]?[0-9]\s?(?:[:mp]\s?[0-5]?[0-9]\s?s?)?\b(\s?-?)"
...
class MonthPattern(BaseTimePattern):
    def get_regex_pattern(self):
        # This match patterns like "tháng 01. 2025", "tháng 12/2025", "02/2025".
        return r"(?i)(?:tháng\s*)?\d{1,2}\s?[\/.-]\s?\d{4}\b"
